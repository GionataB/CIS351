...
public class NumberBase {
	...
	private static int toBase10(String input, int base_in) {
		...
		for( int e = n - 1 ; i < n; i++, e-- ){ //From left to right, so the exponent starts at the string size - 1
			sum += constants_to_int.get(input.charAt(i)) * Math.pow(base_in, e);
		}
	...
	private static String toBaseN(int input, int base_out) {
		...
		// TODO: Fill out this method
		int temp; //The temporary remainder in integer format.
		while( n != 0 ){
			temp = n % base_out;
			sum += constants_from_int.get(temp);
			n = n / base_out;
		}
